  ...
  if (session) {
    return (
      ...
      <div className="container flex w-fit justify-around mx-10 drop-shadow-md shadow-lg rounded-[50px] bg-black bg-opacity-20 py-4">
        <button onClick={async () => await signOut()} className="px-10 max-sm:px-5 py-4 mx-4 min-w-max text-white bg-[#ff2323] rounded-[25px] hover:bg-[#ffffff] hover:text-black transition ease-in-out duration-300">
          تسجيل الخروج
        </button>
        <Link
          href="/"
